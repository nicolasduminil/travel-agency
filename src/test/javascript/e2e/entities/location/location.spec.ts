...
describe('Location e2e test', () => {
  ...
  it('should load Locations', async () => {
    ...
    expect(await locationComponentsPage.getTitle()).to.eq('travelAgencyApp.location.home.title');
  ...
  it('should load create Location page', async () => {
    ...
    expect(await locationUpdatePage.getPageTitle()).to.eq('travelAgencyApp.location.home.createOrEditLabel');
  ...
  it('should create and save Locations', async () => {
    ...
    await promise.all([
      ...
      locationUpdatePage.setStreetNumberInput('streetNumber'),
      ...
      locationUpdatePage.setCountryNameInput('countryName'),
      locationUpdatePage.customerSelectLastOption(),
    ]);
    ...
    expect(await locationUpdatePage.getStreetNumberInput()).to.eq(
      'streetNumber',
    ...
    expect(await locationUpdatePage.getCountryNameInput()).to.eq('countryName', 'Expected CountryName value to be equals to countryName');
  ...
  it('should delete last Location', async () => {
    ...
    expect(await locationDeleteDialog.getDialogTitle()).to.eq('travelAgencyApp.location.delete.question');
