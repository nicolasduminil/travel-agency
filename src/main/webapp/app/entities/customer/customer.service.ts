...
export class CustomerService {
  ...
  protected convertDateFromClient(customer: ICustomer): ICustomer {
    const copy: ICustomer = Object.assign({}, customer, {
      customerBirthDate:
        customer.customerBirthDate && customer.customerBirthDate.isValid() ? customer.customerBirthDate.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.customerBirthDate = res.body.customerBirthDate ? moment(res.body.customerBirthDate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((customer: ICustomer) => {
        customer.customerBirthDate = customer.customerBirthDate ? moment(customer.customerBirthDate) : undefined;
      });
    }
    return res;
  }
